...
`Nini & Yuan` is a Chinese-language fantasy platformer for the web and Android WebView. Version 1.6.2 is a focused reliability patch for the **宋式星图器物幻想 / Song-atlas Night Observatory** release: both protagonists retain their last movement direction when idle, their complete silhouettes remain inside the atlas cell, and Web/PWA plus Android launchers share a mask-safe paired Nini-and-Yuan icon. The game remains offline, local-only, dependency-light, and WebView-ready: two playable characters, fifteen handcrafted chapters, local save data, landscape touch controls, adjustable background music, PWA metadata, and a reproducible debug APK build path.
