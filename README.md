...
`Nini & Yuan` is a Chinese-language fantasy platformer for the web and Android WebView. Version 1.6.1 refines the **宋式星图器物幻想 / Song-atlas Night Observatory** release with faster reversals, input-led camera response, distance-synchronized character motion, front-facing idle poses, stable captured touch controls, canonical trailing-slash URL handling, denser mobile chapter browsing, and a reproducible cross-orientation store-asset flow. The game remains offline, local-only, dependency-light, and WebView-ready: two playable characters, fifteen handcrafted chapters, local save data, landscape touch controls, adjustable background music, PWA metadata, and a reproducible debug APK build path.
