...
assert.ok(["1.2.3", "1.2.4", "1.3.0", "1.3.1", "1.4.0"].includes(pkg.version), `package.json version should be 1.2.3 through 1.4.0 (got ${pkg.version})`);
assert.ok(["1.2.3", "1.2.4", "1.3.0", "1.3.1", "1.4.0"].includes(lock.version), `package-lock.json root version should be 1.2.3 through 1.4.0 (got ${lock.version})`);
assert.match(sw, /CACHE = "nini-yuan-v(1\.2\.(3-starlit-whispers-r3|4-aurora-cartography)|1\.3\.(0-world-2-star-gates|1-typography-copy-fix)|1\.4\.0-world-3-phase-tide)"/, "service worker cache should be at a supported v1.2.3+ key");
...
assert.ok(/versionCode="(6|7|8|9|10)"/.test(androidManifest), "Android versionCode should be 6 through 10");
assert.ok(/versionName="(1\.2\.(3|4)|1\.3\.(0|1)|1\.4\.0)"/.test(androidManifest), "Android versionName should be 1.2.3 through 1.4.0");
