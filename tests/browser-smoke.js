...
function transformFaces(transformA, expectedDirection) {
  return Math.sign(transformA) === expectedDirection;
}
...
async function run() {
  ...
  try {
    ...
    await withPage("desktop gameplay", async (page) => {
      ...
      if (initialIdle.frameIndex !== 0 || !transformFaces(initialIdle.transformA, IDLE_RIGHT_TRANSFORM.nini)) {
      ...
      if (leftIdle.frameIndex !== 0 || !transformFaces(leftIdle.transformA, IDLE_LEFT_TRANSFORM.nini)) {
      ...
      if (rightIdle.frameIndex !== 0 || !transformFaces(rightIdle.transformA, IDLE_RIGHT_TRANSFORM.nini)) {
    ...
    await withPage(
      ...
      async (page) => {
        ...
        if (initial.frameIndex !== 0 || !transformFaces(initial.transformA, IDLE_RIGHT_TRANSFORM.yuan) || initial.character !== "源源") {
        ...
        if (leftIdle.frameIndex !== 0 || !transformFaces(leftIdle.transformA, IDLE_LEFT_TRANSFORM.yuan)) {
