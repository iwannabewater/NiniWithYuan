...
- **Interaction thesis**: **刻金显纹** traces an aged-gold edge on focus or confirmation, then settles a rose or jade inlay; motion never turns an entire surface into glow. From v1.9.0, structural DOM motion stays on ease-out `transform` and `opacity` only, so the field remains calm while controls stay tactile.
