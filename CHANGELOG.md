...
- Added a soft pointer stardust trail on the cover hero region and the brand title for fine-pointer desktop users; trail particles fade and lift over ~720 ms, are capped to 32 active sparks, and are disabled on touch devices, on `(hover: none)` pointers, and under `prefers-reduced-motion`.
...
- Bumped the web package to `1.2.3`, Android `versionCode` to 6 / `versionName` to `1.2.3`, the service worker cache to `nini-yuan-v1.2.3-starlit-whispers`, and the install-prompt manifest description from the legacy platform-jump phrasing to the star-atlas adventure copy used in the cover.
